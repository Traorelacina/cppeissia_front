...
import { Save, Settings, User, Phone, Clock, BookOpen, Upload, X, Image, CheckCircle } from 'lucide-react'
...
// ========================
// COMPOSANT SECTION avec bouton de sauvegarde intégré
// ========================
function ParamSection({ title, icon: Icon, children, onSave, isSaving, savedKey }) {
  return (
    <Paper sx={{ p: 3, mb: 2.5 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2.5, pb: 1.5, borderBottom: '1px solid #dae8df' }}>
        ...
        {/* Bouton Enregistrer propre à la section */}
        <Button
          size="small"
          variant="contained"
          startIcon={
          ...
          disabled={isSaving}
          sx={{
            background: savedKey ? '#388e3c' : '#1B7A3E',
            fontSize: 12,
            fontWeight: 700,
            px: 2,
            py: 0.75,
            ...
            '&.Mui-disabled': { background: '#a5c9b0', color: '#fff' },
          }}
        >
          {isSaving ? 'Enregistrement…' : savedKey ? 'Enregistré ✓' : 'Enregistrer'}
        </Button>
      </Box>
      {children}
    </Paper>
    ...
    <Box>
      ...
      {preview ? (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5 }}>
          <Box sx={{ width: 96, height: 96, borderRadius: '50%', overflow: 'hidden', border: '3px solid #F5A623', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', flexShrink: 0 }}>
            <Box
              component="img"
              src={preview}
          ...
          <Box sx={{ flex: 1 }}>
            <Typography sx={{ fontWeight: 600, fontSize: 13, color: '#0c1a10', mb: 0.5 }}>Photo sélectionnée</Typography>
            <Typography sx={{ fontSize: 11.5, color: '#6b7c70', mb: 1.5, lineHeight: 1.5 }}>
              {preview.startsWith('data:') ? 'Image chargée depuis votre appareil' : 'Image chargée depuis une URL'}
            </Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button size="small" variant="outlined" startIcon={<Upload size={13} />} onClick={() => inputRef.current?.click()}
                sx={{ borderColor: '#1B7A3E', color: '#1B7A3E', fontSize: 12, fontWeight: 600, '&:hover': { background: '#eaf4ee' } }}>
                Changer
              </Button>
              <Button size="small" variant="outlined" startIcon={<X size={13} />} onClick={handleRemove}
        ...
        >
          <Box sx={{ width: 56, height: 56, borderRadius: '50%', background: isDrag ? '#1B7A3E' : '#eaf4ee', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 1.5, transition: 'all 0.2s' }}>
          ...
          <Typography sx={{ fontSize: 12, color: '#6b7c70', lineHeight: 1.6 }}>
            Cliquez ou glissez-déposez une image<br />JPG, PNG, WEBP · 5 Mo max
          </Typography>
          <Button size="small" variant="contained" startIcon={<Upload size={13} />}
...
export default function Parametres() {
  ...
  // État "sauvegardé" par section (affiche ✓ pendant 3s)
  ...
  // ── Mutation générique pour un groupe de clés ──
  const saveMutation = useMutation({
    mutationFn: async ({ sectionKey, keys }) => {
      const promises = keys.map((cle) => {
        const valeur = values[cle] ?? ''
        if (cle === 'photo_directeur' && valeur && valeur.startsWith('data:image')) {
          const formData = new FormData()
          formData.append('valeur', valeur)
          return parametresApi.uploadPhotoDirecteur(formData)
        }
        return parametresApi.update(cle, valeur)
      })
  ...
  // Helpers par section
  const save = (sectionKey, keys) => saveMutation.mutate({ sectionKey, keys })
  const isSaving = (sectionKey) =>
    saveMutation.isPending && saveMutation.variables?.sectionKey === sectionKey
  const isSaved = (sectionKey) => !!savedSections[sectionKey]
  ...
  return (
    <Box>
      <PageTitle
        ...
        subtitle="Chaque section s'enregistre indépendamment. Cliquez sur le bouton correspondant après modification."
      />
      ...
      <Grid container spacing={2.5}>
        ...
        <Grid item xs={12} md={8}>
          ...
          <ParamSection
            ...
            icon={User}
            onSave={() => save('directeur', ['nom_directeur', 'photo_directeur', 'mot_directeur'])}
            isSaving={isSaving('directeur')}
            savedKey={isSaved('directeur')}
          >
            <Grid container spacing={2.5}>
              ...
              <Grid item xs={12}>
                ...
                <Typography sx={{ fontSize: 11, color: '#9ca3af', mt: 1 }}>
                  L'image sera affichée sur la page "Mot du Directeur" et sur la page d'accueil.
                </Typography>
              </Grid>
          ...
          <ParamSection
            ...
            icon={Phone}
            onSave={() => save('coordonnees', ['telephone', 'email', 'adresse'])}
            isSaving={isSaving('coordonnees')}
            savedKey={isSaved('coordonnees')}
          >
          ...
          <ParamSection
            ...
            icon={Clock}
            onSave={() => save('horaires', ['horaires'])}
            isSaving={isSaving('horaires')}
            savedKey={isSaved('horaires')}
          >
            <TextField
        ...
        <Grid item xs={12} md={4}>
          ...
          {/* ── 4. ANNÉE SCOLAIRE & INSCRIPTIONS ── */}
          <ParamSection
            title="Année scolaire & Inscriptions"
            icon={BookOpen}
            onSave={() => save('inscriptions', ['annee_scolaire_courante', 'date_rentree', 'scolarite_montant', 'inscriptions_ouvertes'])}
            isSaving={isSaving('inscriptions')}
            savedKey={isSaved('inscriptions')}
          >
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box
                sx={{
                  ...
                  p: 2, borderRadius: '10px', border: '1px solid #dae8df',
              ...
              >
                <Box>
                  ...
                  <Typography sx={{ fontSize: 11.5, color: '#6b7c70', mt: 0.25 }}>
                    Affiche ou masque la bannière publique
                  </Typography>
                </Box>
                <Button
                  size="small"
          ...
          <Box
            sx={{
              background: '#eaf4ee', border: '1px solid rgba(27,122,62,0.2)',
              borderRadius: '12px', p: 2, fontSize: 12.5, color: '#2d3a30', lineHeight: 1.7,
            }}
          >
            ...
            Chaque section est indépendante. Modifiez un bloc puis cliquez sur son bouton
            <strong> Enregistrer</strong> — les autres sections ne sont pas affectées.
